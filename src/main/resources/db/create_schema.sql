...
-- Execute este script ANTES de subir a aplicação para garantir nomes padronizados
-- Ou rode após dropar o schema para recriar limpo

CREATE DATABASE IF NOT EXISTS `busvans` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `busvans`;
...
-- Drop tables se existirem (para recriar limpo)
DROP TABLE IF EXISTS `empresa_veiculo`;
DROP TABLE IF EXISTS `passagem_ticket`;
DROP TABLE IF EXISTS `veiculo_veiculo_rota`;
DROP TABLE IF EXISTS `rota_cidade`;
DROP TABLE IF EXISTS `veiculo_rota`;
DROP TABLE IF EXISTS `preco_passagem`;
DROP TABLE IF EXISTS `localizacao_veiculo`;
...
-- Adiciona FK de veiculo -> localizacao_veiculo após a tabela existir
...
-- Criação concluída! Agora rode seed_workbench.sql para popular dados.
